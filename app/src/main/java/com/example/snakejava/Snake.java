...
public class Snake {
    ...
    public void updateskin(Bitmap bmp)
    {
        this.bm = bmp;
        bm_body_bottom_left = Bitmap.createBitmap(bm,0,0,GameView.sizeOfMap,GameView.sizeOfMap);
        bm_body_bottom_right = Bitmap.createBitmap(bm,GameView.sizeOfMap,0,GameView.sizeOfMap,GameView.sizeOfMap);
    ...
    public void removePart()
    {
        ...
        if(p.getBm()==bm_tail_right)
        {
            this.arrPartSnake.remove(new PartSnake(bm_tail_right, p.getX()- GameView.sizeOfMap, p.getY()));
        }
        else if(p.getBm()==bm_tail_left)
        {
            this.arrPartSnake.remove(new PartSnake(bm_tail_left, p.getX()+ GameView.sizeOfMap, p.getY()));
        }
        else if(p.getBm()==bm_tail_up)
        {
            this.arrPartSnake.remove(new PartSnake(bm_tail_up, p.getX() , p.getY()+GameView.sizeOfMap));
        }
        else if(p.getBm()==bm_tail_down)
        {
            this.arrPartSnake.remove(new PartSnake(bm_tail_right, p.getX(), p.getY()- GameView.sizeOfMap));
        }
    }
